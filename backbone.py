...
class Clip(nn.Module):
    def __init__(self, args, emb_size):
        ...
        # self.att = AttentionPool2d(224 // 32, 2048, 4, 128)
        self.args = args
        self.emb_size = emb_size
        self.avgpool = nn.AvgPool2d(6)
        self.maxpool = nn.MaxPool2d(kernel_size=2)
        self.layer_last = nn.Sequential(nn.Linear(in_features=2048,
                                                  out_features=self.emb_size,
                                                  bias=True),
                                        nn.BatchNorm1d(self.emb_size))
        self.layer_second = nn.Sequential(nn.Linear(in_features=1024*49,
                                                    out_features=self.emb_size,
                                                    bias=True),
                                          nn.BatchNorm1d(self.emb_size))
        self.model, _ = clip.load('RN50', device=self.args.device)
        for p in self.model.parameters():
            p.requires_grad = False

    def forward(self, input):

        _, last_sec_layer, last_layer = self.model.encode_image(input)      # Penultimate: [10, 1024, 14, 14]; final: [10, 2048, 7, 7]
        # feat = self.att(last_layer)
        last_sec_layer = last_sec_layer.float()
        last_layer = last_layer.float()

        last_layer = self.avgpool(last_layer)
        last_layer = self.layer_last(last_layer.view(last_sec_layer.size(0), -1))

        last_sec_layer = self.maxpool(last_sec_layer)   #10,1024,7,7
        last_sec_layer = self.layer_second(last_sec_layer.view(last_sec_layer.size(0), -1))

        out = []
        out.append(last_layer)
        out.append(last_sec_layer)
