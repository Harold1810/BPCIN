...
class FSL_VQA(data.Dataset):
    def __init__(self, root, partition='train', category='COCO', max_token=15, token_to_ix=None):
        ...
        if category not in split_dirs:
            raise ValueError('Unsupported dataset category: {}'.format(category))

        IMAGE_PATH = os.path.join(self.root, image_dirs[category])
        txt_path = os.path.join(
            self.root, split_dirs[category], '{}.pth'.format(self.partition)
        )
        self.img_feat_path_list = glob.glob(
            os.path.join(self.root, 'vinvl', '*.npz')
        )

        # {image id} -> {image feature absolutely path}
        self.iid_to_img_feat_path = img_feat_path_load(self.img_feat_path_list)   # Map each image ID to its feature path.

        self.c = np.load(
            os.path.join(self.root, 'predict_vinvl.npy'), allow_pickle=True
        ).item()
        ...
        # self.embbeding = nn.Embedding(num_embeddings=len(self.cls_id),
        #                               embedding_dim=300)
    ...
    def __getitem__(self, index):
        ...
        # else:
        #     vinvl_feat = self.f['%d_features' % img_id]
        #     vinvl_feat = proc_img_feat(vinvl_feat, 50)

        cls = self.c[str(img_id)]

        cls_id = proc_class(cls, self.cls_id, 50)
        # cls_emb_id = torch.tensor(cls_id)
        # cls_emb = self.embbeding(cls_emb_id)
    ...
    # def proc_class(self, cls, num_pad):
    #     if len(cls) > num_pad:
    #         cls = cls[:num_pad]
    #
    #     cls = cls + [0] * (num_pad - len(cls))
    #
    #     return cls
    ...
    def get_task_batch(self):
        ...
        for t_idx in range(self.num_tasks):
            ...
            for c_idx in range(self.num_ways):
                ...
                class_data_list = [self.dataset[img_idx][0] for img_idx in data_idx]        # Image data
                class_ques_list = [self.dataset[img_idx][1] for img_idx in data_idx]
                # vinvl_path = [self.iid_to_img_feat_path[str(img_idx)] for img_idx in data_idx]
                ...
                for i_idx in range(self.num_shots):
                    # set data
                    support_data[i_idx + c_idx * self.num_shots][t_idx] = self.transform(class_data_list[i_idx])        # Convert image data to image features.
...
def tokenize(total_words, use_glove=True):
    ...
    # with open('./que.json', 'w') as json_file:
    #     json.dump(token_to_ix, json_file)
...
def tokenize_class(total_words, ix, emb, use_glove=True):
    ...
    # with open('./que+cls.json', 'w') as json_file:
    #     json.dump(token_to_ix, json_file)
...
if __name__ == '__main__':
    ...
    dataset_train = FSL_VQA(root='/nvme3/home/xsq/data/fsl_vqa/COCO_QA', partition='train')
