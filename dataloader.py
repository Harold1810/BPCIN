...
class FSL_VQA(data.Dataset):
    def __init__(self, root, partition='train', category='COCO', max_token=15, token_to_ix=None):
        ...
        if category == 'COCO':
            txt_path = os.path.join(self.root, 'object', '{}_fpait.pth'.format(self.partition))
        else:
        ...
        if category == 'VQA':
            ...
            # self.f = h5py.File('/home/public/caption/caption/coco2014/feats/COCO2014_VinVL.hdf5', 'r')
            self.img_feat_path_list = glob.glob('/home/public/caption/caption/coco2014/feats/vinvl/' + '*.npz')

        # {image id} -> {image feature absolutely path}
        self.iid_to_img_feat_path = img_feat_path_load(self.img_feat_path_list)   # 返回一个字典

        self.c = np.load('/home/public/caption/caption/coco2014/feats/predict.npy', allow_pickle=True).item()
    ...
    def get_task_batch(self):
        ...
        for t_idx in range(self.num_tasks):
            ...
            for c_idx in range(self.num_ways):
                ...
                class_data_list = [self.dataset[img_idx][0] for img_idx in data_idx]        #图像信息
                ...
                for i_idx in range(self.num_shots):
                    # set data
                    support_data[i_idx + c_idx * self.num_shots][t_idx] = self.transform(class_data_list[i_idx])        #图像信息——>图像特征
...
if __name__ == '__main__':
    ...
    dataset_train = FSL_VQA(root='/home/public/vqa_data/FSL_VQA/COCO_QA', partition='train')
