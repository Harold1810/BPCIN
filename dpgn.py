...
class PointSimilarity(nn.Module):
    def __init__(self, in_c, base_c, dropout=0.0):
        ...
        self.point_sim_transform = nn.Sequential(*layer_list)  # Two-layer Conv2d-BN-ReLU similarity module described in the paper.
        # self.relu = nn.ReLU()
    ...
    def forward(self, vp_last_gen, ep_last_gen, distance_metric):
        ...
        ep_ij = torch.sigmoid(self.point_sim_transform(trans_similarity))   # Compute edge weights from pairwise similarities.
...
#
#         # Obtain point-graph node features for each batch.
#         # batch_tensors = torch.unbind(vp_last_gen, dim=0)
#         # Apply the diffusion map to each graph in the batch.
...
class P2DAgg(nn.Module):
    ...
    def forward(self, point_edge, distribution_node):
        # point_edge:[10,10,10]
        # Update the distribution-graph node features.
        ...
        distribution_node = self.p2d_transform(distribution_node)   # Transform with a linear layer and LeakyReLU.
        distribution_node = distribution_node.view(meta_batch, num_sample, -1)
...
# Distribution similarities define the distribution-graph edges.
class DistributionSimilarity(nn.Module):
    def __init__(self, in_c, base_c, dropout=0.0):
        ...
        self.point_sim_transform = nn.Sequential(*layer_list)  # Update distribution-graph edge similarities.
...
class D2PAgg(nn.Module):
    def __init__(self, in_c, base_c, dropout=0.0):
        ...
        self.point_node_transform = nn.Sequential(*layer_list)  # Fuse and reduce point-graph features from consecutive generations.
        self.linear1 = nn.Linear(base_c*2, base_c)
        self.linear2 = nn.Linear(300, base_c)

    def forward(self, distribution_edge, point_node):
        # Update the point-graph node features.
        # get size
        ...
        node_feat = self.point_node_transform(node_feat.unsqueeze(-1))  # Fuse the previous and updated point-graph nodes.
...
class SemPointUpdate(nn.Module):
    ...
    def forward(self, distribution_edge, instance_edge, point_node):
        ...
        # Create a one-minus-identity mask of shape [batch_size, node_size, node_size].
        diag_mask = 1.0 - torch.eye(num_sample).unsqueeze(0).repeat(meta_batch, 1, 1).to(distribution_edge.get_device())

        # Set diagonal edge weights to zero and normalize the remaining edges.
        edge_feat = F.normalize(distribution_edge * diag_mask, p=1, dim=-1)  # [25,10,10]

        # Multiply distribution-graph edges by point-graph nodes to compute attention.
        aggr_feat = torch.bmm(edge_feat, point_node)

        # Concatenate the recomputed features with the point features from the previous iteration.
        ...
        # Pass the result through the D2P network.
...
class LinearTransformation(nn.Module):          #P2DAgg
    ...
    def forward(self, point_edge, distribution_node):
        ...
        # Concatenate the edge set with the distribution-node features from the previous iteration.
        ...
        # Pass the result through the P2D network.
...
class DPGN(nn.Module):
    def __init__(self, num_generations, dropout, num_way, num_shot, num_support_sample, num_sample, loss_indicator, point_metric, distribution_metric):
        ...
        self.add_module('initial_edge', P_Sim)  # Register the initial edge module.
        # self.add_module('l_initial_edge', INIT)
        for l in range(self.generation): # self.generation => 6
            D2P = D2PAgg(128*2, 128, dropout=self.dropout if l < self.generation-1 else 0.0)  # Disable dropout in the final generation.
    ...
    def get_proto_node_with_query(self, node_feat, edge_feat):
        ...
        # Find the support index with the highest query similarity from the subgraph edges.
        ...
        index = torch.argmax(query_similarity, dim=-2)  # [25, 5]/[25, 25]: index of the node most similar to each support node.
        ...
        return protos   # Fuse the N-way query nodes most similar to support nodes into class prototypes.
    ...
    def get_proto_distribution(self, protos, node_feat):
        ...
        for sample_index in range(0, num_samples):  # Compute the similarity between every sample and each prototype.
    ...
    # def get_semantic_proto_distribution(self, protos, cls):
    #     # cls = cls.sum(-2)
    #
    #     proto_distribution = None  # [25,10,5]   node-proto distribution
    #     for sample_index in range(0, self.num_sample):  # Compute the similarity between every sample and each prototype.
    ...
    #
    #     return proto_distribution  # [batch, sample, 10, 50]: similarities between 50 object classes and each sample question.
    ...
    def forward(self, middle_node, point_node, que_data, distribution_node, vinvl_feat, cls, que_embedding, cls_embedding):
        ...
        # print(cls)      # cls: [batch, sample, 50, 300] features after the LSTM and self-attention layers
        ...
        sem_edge_similarities = []                 # Semantic graph edge loss
        ...
        for l in range(self.generation):
            ...
            # Fuse VinVL and CLIP features to enhance the visual representation.
            ...
            # Compare enhanced nodes with support nodes to obtain distribution nodes.
            ...
            # (9) Update the semantic point nodes.
