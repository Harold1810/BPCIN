...
class DPGNTrainer(object):
    def __init__(self, enc_module, gnn_module, que_enc, data_loader, log, arg, config, best_step, cls_enc):
        ...
        self.arg = arg  # Runtime arguments
        self.config = config  # Training configuration
        self.train_opt = config['train_config']  # Training options
        self.eval_opt = config['eval_config']  # Evaluation options

        # initialize variables
        self.tensors = allocate_tensors()  # Initialize training tensors.
    ...
    def train(self):
        ...
        for iteration, batch in enumerate(self.data_loader['train']()):
            ...
            all_label_in_edge, node_feature_gd, all_vinvl, all_cls = initialize_nodes_edges(batch,
                                                                      num_supports,  # 5
                                                                      self.tensors,  # Initialized training tensors
                                                                      self.train_opt['batch_size'],   # 25
                                                                      self.train_opt['num_queries'],  # 1
            ...
            total_loss, query_node_cls_acc_generations, query_edge_loss_generations = \
                self.compute_train_loss_pred(all_label_in_edge,   # [25,10,10]
                                             point_similarity,    # Six [25, 10, 10] tensors
                                             node_similarity_l2,  # Six [25, 10, 10] tensors
                                             query_edge_mask,     # [25,10,10]
                                             evaluation_mask,     # [25,10,10]
                                             num_supports,        # 5
                                             support_label,       # [25,5]
                                             query_label,         # [25,5]
                                             distribution_similarities, # Six [25, 10, 10] tensors
...
def main():
    ...
    parser.add_argument('--dataset_root', type=str, default=os.path.join('.', 'data'),
                        help='root directory containing splits, images, and VinVL features')
    ...
    train_opt = config['train_config']   # Training options
    eval_opt = config['eval_config']     # Evaluation options
    ...
    # dataset_valid = dataset(root=args_opt.dataset_root, category=config['dataset_name'],
    #                         partition='val', token_to_ix=dataset_train.token_to_ix)
