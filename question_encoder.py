...
import torch
from mca import SA
from retention import Ret
from retention import RetNetRelPos
...
class Question_Encoder(nn.Module):
    def __init__(self, pretrained_emb, token_size, emb_size=300, out_dim=300):
        ...
        # self.layer_last2 = nn.Sequential(nn.Linear(in_features=512,
        #                                           out_features=out_dim,
        #                                           bias=True),
        #                                 nn.BatchNorm1d(out_dim))
        self.sa = SA()
    ...
    def forward(self, all_que):
        ...
        # for _ in range(2):
        #     lang_feat = self.att(lang_feat, None)
        # lang_feat = self.ret(lang_feat)

        lang_feat = self.sa(lang_feat, None)


        lang_feat = lang_feat.reshape(lang_feat.size(0), -1)
        # cls_feat = cls_feat.reshape(batch*50, 512)
        # 15*512 -> out_dim
        lang_feat = self.layer_last1(lang_feat)
        # cls_feat = self.layer_last2(cls_feat)
        # cls_feat = cls_feat.reshape(batch, 50, -1)
