...
class MultiScaleRetention(nn.Module):
    def __init__(self, embed_dim, num_heads, value_factor=2, gate_fn="swish"):
        super().__init__()
        ...
        self.v_proj = MultiwayWrapper(nn.Linear(embed_dim, embed_dim * self.factor, bias=True))
        self.g_proj = MultiwayWrapper(nn.Linear(embed_dim, embed_dim * self.factor, bias=True))

        self.out_proj = MultiwayWrapper(nn.Linear(embed_dim * self.factor, embed_dim, bias=True))

        self.group_norm = MultiwayWrapper(LayerNorm(self.head_dim, eps=1e-6))
...
class RetNetRelPos(nn.Module):
    def __init__(self):
        super().__init__()
        angle = 1.0 / (10000 ** torch.linspace(0, 1, 512 // 4 // 2))
        angle = angle.unsqueeze(-1).repeat(1, 2).flatten()
        decay = torch.log(1 - 2 ** (-5 - torch.arange(4, dtype=torch.float)))
...
class Ret(nn.Module):
    def __init__(self):
        ...
        self.ret = MultiScaleRetention(512, 4)
        self.dropout1 = Dropout(0.1)
        self.ffn = FFN(512, 1024, 512)
        self.norm1 = LayerNorm(512)
        self.dropout2 = Dropout(0.1)
        self.norm2 = LayerNorm(512)
        self.repo = RetNetRelPos()

    def forward(self, x):
        repo = self.repo(15)
        x = self.norm1(self.dropout1(self.ret(x, repo)) + x)
        output = self.norm2(x + self.dropout2(self.ffn(x)))
