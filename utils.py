...
# Copy the current batch into the initialized training tensors.
...
# Convert labels into edge labels.
...
# Generate masks for the support and query sets.
def preprocessing(num_ways, num_shots, num_queries, batch_size, device):
    ...
    support_edge_mask[:, :num_supports, :num_supports] = 1  # Set the top-left support block of the 10 x 10 matrix to one.
    # print(support_edge_mask)
    query_edge_mask = 1 - support_edge_mask  # Complement of support_edge_mask.
    evaluation_mask = torch.ones(batch_size, num_samples, num_samples).to(device)  # All-ones evaluation mask.

    return num_supports, num_samples, query_edge_mask, evaluation_mask

# Initialize graph nodes and edge labels.
def initialize_nodes_edges(batch, num_supports, tensors, batch_size, num_queries, num_ways, device):
    ...
    node_gd_init_support = label2edge(support_label, device)  # [25, 5, 5]: identity matrices that initialize support distribution nodes.
    node_gd_init_query = (torch.ones([batch_size, num_queries * num_ways, num_supports])
                          * torch.tensor(1. / num_supports)).to(device)  # [25, 5, 5]: uniformly initialize query distribution nodes.
    node_feature_gd = torch.cat([node_gd_init_support, node_gd_init_query], dim=1)  # [25, 10, 5]: each node is related only to support nodes,
                                                                                    # so the shape is 10 x 5 instead of 10 x 10.
    ...
    all_label_in_edge = label2edge(all_label, device)  # [25, 10, 10] identity matrices for one-shot tasks
...
def backbone_two_stage_initialization(full_data, encoder):
    ...
    for data in full_data.chunk(full_data.size(1), dim=1):
        ...
        encoded_result = encoder(data.squeeze(1))  # Two [25, 128] tensors; COCO: [10, 128] final and penultimate features.
    ...
    last_layer_data = torch.stack(last_layer_data_temp, dim=1)  # [25, 10, 128]: stack temporary features along a new dimension.
...
# Extract question and object-class features.
